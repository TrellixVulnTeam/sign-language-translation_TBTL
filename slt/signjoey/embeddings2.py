...
class SpatialEmbeddings(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.token_layer = nn.Linear(self.max_token_len, 1)
        self.embedding_dim = embedding_dim
    ...
    def forward(
        ...
        seq_size = x.shape[1]
        x_tkn = torch.zeros((batch_size, seq_size + max_num_token, self.input_size), device=torch.device('cuda')) # (32,578,1024)
        ...
        for i in range(batch_size):
            for n in range(num_token[i]):
                ...
                x_tkn[i,breakpoint_i[n]+n:breakpoint_i[n+1]+n,:] = action_token
                x_tkn[i,n:breakpoint_i[n+1]+n,:] = tmp
        x_tkn = x_tkn.to("cuda")
