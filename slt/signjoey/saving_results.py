...
try:
    df = pd.read(saving_path + 'test_result.csv')
except:
    df = pd.DataFrame(columns=['start_day', 'train_no', 'eval_metric', 'learning_rate', 'batch_size', 'early_stopping_patience', 'weight_decay', 'dropout', 'Epoch', 'Step', 'WER', 'BLEU-4'])
...
for number in train_list:
    ...
    if train_no not in df['train_no']:
        print(f'appending {train_no}')
        log = load_log(log_path + train_no + '.log')
        start_day, eval_metric, learning_rate, batch_size, early_stopping_patience, weight_decay, dropout= '', '', '', '', '', '', ''
        cfg_done = False
        for line in log:
            ...
            elif 'cfg.model.encoder.embeddings.dropout' in line and not dropout:
                dropout = line.split(': ')[-1].split('\n')[0]
                cfg_done = True
            elif cfg_done:
                break
        ...
        record = [start_day, train_no, eval_metric, learning_rate, batch_size, early_stopping_patience, weight_decay, dropout, Epoch, Step, WER, BLEU]
        df_new.loc[len(df_new)] = record
df_new.to_csv(saving_path + 'test_result.csv')
